...
$result=mysqli_query($con,"SELECT first_name,last_name,age,gender,parent_name,house_name,state,district,email,phone_no,license_type,date_of_issue,expiriry_date,blood,paystatus,date FROM `tbl_drivinglicense` where reg_id=$regid") or die(mysqli_error($con));
...
$pdf->Cell(176, 5, 'TRANSPORT DEPARTMENT', 0, 0, 'C');
  ...
  $pdf->Ln();
  $pdf->Ln();
$pdf->Cell(176, 5, 'E-RECEIPT FOR ONLINE DRIVING LICENSE', 0, 0, 'C');
  $pdf->Ln();
...
$pdf->Cell(0,12,'Paid Amount : 900',0,1);
$pdf->Cell(0,12,'Payment Date : '. $row['date'],0,1);
$pdf->Ln();	
$pdf->Ln();
$pdf->Ln();	
$pdf -> Line(10, 249, 190,249);
$pdf->Image("digital.jpg", 100, 251, 100);
