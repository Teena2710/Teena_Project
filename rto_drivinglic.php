...
$sql=mysqli_query($con,"SELECT * FROM `tbl_drivinglicense` where is_approved='YES' order by driving_id ASC") or die(mysqli_error($con));
if(isset($_POST['approve']))
{
    $driving_id=$_POST['driving_id'];
    mysqli_query($con,"UPDATE `tbl_drivinglicense` SET `is_approved` = 'APPROVED' WHERE `tbl_drivinglicense`.`driving_id` = $driving_id;") or die(mysqli_error($con));
    // echo "<script>alert('Approved successfully');window.location.href='appointment.php';</script>";
    $_SESSION['driv_id']=$driving_id;
