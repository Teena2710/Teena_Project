...
$result=mysqli_query($con,"SELECT first_name,last_name,age,gender,parent_name, `house_name`, `state`,`district`,email,phone_no,license_type,date_of_issue,expiriy_date,blood,paystatus,pay_date FROM `tbl_duplicatelicense` where driving_id='$b'") or die(mysqli_error($con));
...
$pdf->Cell(176, 5, 'TRANSPORT DEPARTMENT', 0, 0, 'C');
  ...
  $pdf->Ln();
  $pdf->Ln();
$pdf->Cell(176, 5, 'E-RECEIPT FOR DUPLICATE LICENSE', 0, 0, 'C');
  $pdf->Ln();
...
$pdf->Cell(0,12,'Expiriry Date : '. $row['expiriy_date'],0,1);
$pdf->Cell(0,12,'Blood : '. $row['blood'],0,1);
$pdf->Cell(0,12,'Pay Status : '. $row['paystatus'],0,1);
$pdf->Cell(0,12,'Paid Amount : 500',0,1);
$pdf->Cell(0,12,'Payment Date : '. $row['pay_date'],0,1);
$pdf->Ln(); 
$pdf->Ln();
$pdf->Ln(); 
$pdf -> Line(10, 249, 190,249);
$pdf->Image("digital.jpg", 90, 251, 100);
$pdf->Image("qr_code.png", 30, 251, 40);
$pdf->Output();
?>